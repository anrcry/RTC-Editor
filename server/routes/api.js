...
// Check the access level of a particular user
router.get('/users/:username/documents/:documentUuid/access', isAuthenticated, hasReadPermission, async function (req, res, next) {
  ...
  if (!username) {
    res.status(400).json({ success: false, message: 'The username is required.' });
  }

  const documentUuid = req.params.documentUuid;

  if (!documentUuid) {
    res.status(400).json({ success: false, message: 'The documentUuid is required.' });
  }
  
  const rows = await req.db.select('permissions').from('collaborators').
  where('document', documentUuid).andWhere('user', username);
  
  if (rows.length === 0) {
    return res.status(404).json({ success: false, message: 'No user was found with that username or document does not exist or user is not a collaborator of the document.' });
  }

  const access = permissionsToAccess(rows[0].permissions);
